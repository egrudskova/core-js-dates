...
function getNextFriday(date) {
  ...
  const day = newDate.getDate() + addDate;
  newDate.setDate(day);
  return newDate;
}
...
function getCountWeekendsInMonth(month, year) {
  ...
  for (let i = 1; i <= lastDate; i += 1) {
    const curr = new Date(year, month - 1, i);
    if ([0, 6].includes(curr.getDay())) count += 1;
  }
  return count;
}
...
function getNextFridayThe13th(date) {
  ...
  fridayThe13th.setDate(13);

  while (fridayThe13th.getDay() !== 5) {
    fridayThe13th.setMonth(fridayThe13th.getMonth() + 1);
  }
